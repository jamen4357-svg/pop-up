...
class AnkiUtils:
    ...
    def get_question(self, card):
        if hasattr(card, 'question') and callable(getattr(card, 'question')):
    ...
    def get_answer(self, card):
        if hasattr(card, 'answer') and callable(getattr(card, 'answer')):
            answer = card.answer()
        else:
    ...
    def get_current_card(self):
        ...
        if card is not None:
            ...
            # Use Anki's built-in rendering for question and answer
            # This should properly handle media references and Anki markup
